...
Published copies (served by `make docs-up`):
- `docs/spec/v2/openapi.yaml`
- `docs/spec/v2/semantic.md`
...
- Best-effort derivations:
  ...
  - `zones[].roomRids`: derived from `zone.children[]` (may contain `room`, `light`, and/or `device` refs depending on the bridge) by mapping to room rids via the rule above.
  - Both fields MAY be `null` when the gateway cannot derive a mapping (clients should treat `null` as “unknown”).
